...
class PuLearning:
    ...
    def setup_data(self, data: DataFrame, prop, TARGET):  # TODO how to give prop and target nice?
        ...
        split_id_dir_path = configuration.input_dir / f"{configuration.data_prefix}{TARGET}_{prop}"
        ...
        leaveoutdf = experimental_df.sample(frac=configuration.config['General']['test_ratio']*0.5,
        ...
        for it in range(configuration.config['General']['num_iter']):
            # select labeled test data: part positive labeled data and part labeled leaveout data
            testdf1 = positive_df.sample(frac=configuration.config['General']['test_ratio'], random_state=it) # select "test_ratio" positives, frac see later
